...
El mapa utiliza **Maps Embed API** en modo `place`: busca por nombre el perfil seleccionado, sin guardar coordenadas inventadas. Para configurarlo, habilita Maps Embed API y facturación en Google Cloud, coloca una clave de navegador en `.env.local` como `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=...` y reinicia el servidor. Restringe la clave por referente HTTP (`localhost:5173/*` para la demo local y el dominio publicado si se usa) y por API (solo Maps Embed API). Al ser una clave de navegador, será visible en las peticiones del mapa; nunca uses aquí una clave privada. La ubicación que devuelve Google debe confirmarse antes de visitar.
